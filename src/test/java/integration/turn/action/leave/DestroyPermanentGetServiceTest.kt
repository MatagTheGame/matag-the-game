...
class DestroyPermanentGetServiceTest {
    ...
    fun testDestroy() {
        ...
        Assertions.assertThat<CardInstance?>(gameStatus.getPlayer1().getGraveyard().getCards()).contains(cardInstance)
    }
    ...
    fun testDestroyIndestructibleDoesNotDestroyIt() {
        ...
        val cardInstance =
            cardInstanceFactory!!.create(gameStatus, 61, cards!!.get("Canopy Spider"), "player-name", "player-name")
        cardInstance.getModifiers().getModifiersUntilEndOfTurn().getExtraAbilities()
        ...
        // Then
        Assertions.assertThat(destroyed).isFalse()
        Assertions.assertThat<CardInstance?>(gameStatus.getPlayer1().getBattlefield().getCards()).contains(cardInstance)
        Assertions.assertThat(gameStatus.getPlayer1().getGraveyard().size()).isEqualTo(0)
    }
    ...
    fun testReduceToughnessToZeroOfIndestructibleDestroysIt() {
        ...
        aCreature.getModifiers().getModifiersUntilEndOfTurn().getExtraAbilities()
        ...
        // Then
        Assertions.assertThat(destroyed).isFalse()
        Assertions.assertThat<CardInstance?>(gameStatus.getPlayer1().getBattlefield().getCards()).contains(aCreature)
        Assertions.assertThat(gameStatus.getPlayer1().getGraveyard().size()).isEqualTo(0)
    }
}