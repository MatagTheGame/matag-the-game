...
@Import(CastInstantPoweringCreatureDuringCombatTest.InitTestServiceForTest::class)
class CastInstantPoweringCreatureDuringCombatTest(var initService: InitTestService) : AbstractApplicationTest() {
    ...
    fun castInstantPoweringCreatureDuringCombat() {
        ...
        browser.player2().getBattlefieldHelper(PlayerType.PLAYER, BattlefieldHelper.COMBAT_LINE)
            .getFirstCard(cards.get("Bartizan Bats")).hasPowerAndToughness("4/4")
        browser.player2().getPhaseHelper().matches(DeclareBlockersPhase.DB, PlayerType.PLAYER)
